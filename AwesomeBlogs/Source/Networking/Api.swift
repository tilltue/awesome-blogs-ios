...
enum Api {
    static func getFeeds(group: AwesomeBlogs.Group, force: Bool = false) -> Single<[Entry]> {
        var remote = Service.shared.container
            .resolve(MoyaProvider<AwesomeBlogsRemoteSource>.self)!.singleRequest(.feeds(group: group))
    ...
    static func readFeeds(link: String) -> Observable<Void> {
        return Service.shared.container.resolve(MoyaProvider<AwesomeBlogsRemoteSource>.self)!.rx.request(.read(link: link)).asObservable().map{ _ in return () }
    }
}
